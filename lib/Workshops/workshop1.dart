...
//    });
  ...
  Future<void> _pay(
      BuildContext context, String ename, FirebaseUser user) async {
    // The pay function
    await http.post('https://us-central1-srijanju20.cloudfunctions.net/app/pay',
        body: '{"name": "' + ename + '"}',
        headers: {
          'Authorization': 'Bearer ${(await user.getIdToken()).token}',
        }).then((response) async {
      print("response pay " + response.body.toString());
